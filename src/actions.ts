...
actionManager.addAction((ctx, next) => {
  ...
  if (ctx.actionType && [...userActions, ...adminOnlyActions].includes(ctx.actionType) && !ctx.currentUser) {
...
actionManager.addAction((ctx, next) => {
  ...
  const [role, name, password] = ctx.args as [string, string, string]
...
actionManager.addAction((ctx, next) => {
  ...
  const [name, password] = ctx.args as [string, string];
...
actionManager.addAction((ctx, next) => {
  ...
  const books = ctx.library.books.list() as TBook[];
  if (books.length === 0) {
...
actionManager.addAction((ctx, next) => {
  ...
  const [bookName, author] = ctx.args as [string, string];
  const books = ctx.library.books.search({ bookName, author }) as TBook[];
...
actionManager.addAction((ctx, next) => {
  ...
  const [bookName, author, amount] = ctx.args as [string, string, string];
